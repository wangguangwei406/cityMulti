(function ($) {
    $.fn.hsCheckData = function (options) {
        ...
        $(this).click(function () {
            ...
            $("[name='datali']>div").click(function () {
                ...
                if (!opts.isShowCheckBox) {
                    ...
                    if(data_value.length > 30){
                        data_value = data_value.substring(0,30);
                        console.log(data_value.length);
                    }else{
                        console.log(data_value.length + '2222222');
                    }
            ...
            if (!opts.isShowCheckBox) {
                ...
                $("#" + id + "_btn").click(function () {
                    ...
                    if(data_value.length > 30){
                        data_value = data_value.substring(0,18)+'...';
                        console.log(data_value.length);
                    }
        ...
        function dataListHtmlFun() {
            ...
            //1、解析json数据
            var data = getDataHtml(opts.data);
            html += data + "</div>"
            return html;
        }
        //绑定默认值
        ...
        function childDataHtml(json) {
            ...
            for (var i = 0; i < json.length; i++) {
                console.log(json[i]);
                if("childCity" in json[i]){
                    var childIsNUll = false;
                    childDataHtml(json[i]);
                }
                for (var key in json[i]) {
                    if (json[i][key].toString().indexOf("object") == -1) {
                        if (childIsNUll == false) {
                            ...
                            html += "<span>" + json[i][key] + "</span>";
                            html += "</div></li>"
                        }
                    } else {
                        html += childDataHtml(json[i][key]);
                        html += "</li>"
                    }
                }
            }
